...
void WriteGpioPin(char pin, char state)
{
    if(state == 0)
        ClearGpioPin(pin);
    else
...
static char* Res = "1x1";
static char* AlwaysInit = "on";

module_param(Res, charp, 0000);
module_param(AlwaysInit, charp, 0000);

static unsigned char operations[8];
static unsigned char data[8];

unsigned char Rows;
unsigned char Cols;
unsigned char numOfDisplays;

/*Upisuje u ¿ift registar bit po bit*/
void WriteWord(unsigned short word)
{
    ...
    for(i = 16; i > 0; i--)
    {
        ...
        WriteGpioPin(CLK, LOW);
        
        if(word & mask)
            WriteGpioPin(DATA, HIGH);
        else
            WriteGpioPin(DATA, LOW);

        WriteGpioPin(CLK, HIGH);
    }
    // Na uzlaznu ivicu kloka se upisuje jedan bit u ¿ift registar i pomera
    // sve bite u registru za jesno mesto

}

/*Nakon upisa u sve displeje samo ispi¿e to stanje na displeje*/
void WriteToScreenByRows(void)
{
    unsigned char i;

    WriteGpioPin(LOAD, HIGH);  

    for(i = numOfDisplays; i > 0; i--)
        WriteWord((operations[i - 1] << 8) + data[i - 1]);

    WriteGpioPin(LOAD, LOW);  
    WriteGpioPin(LOAD, HIGH);  

    // Na silaznu ivicu LOAD flega se iz ¿ift registra "utiskuju" biti u 
    // ostale registre, koji nakon inicijalizacije ispisuje stanje registra na displej
}

/*Na osnovu matrice koja se pro¿ita iscrtava celu matricu na displej veli¿ine 256*/
void printDispFour(void)
{
    unsigned char i, j;
    for(i = 0; i < DISP_UNIT; i++)
    {
        for(j = 0; j < numOfDisplays; j++)
        {
            operations[j] = OP_DIGIT0 + i; 
            data[j] = 0; 
        }
        
        for(j = 0; j < DISP_UNIT; j++)
        {
            data[2] += gpio_driver_buffer[j*DISP_UNIT*Cols + i] << j; 
            data[3] += gpio_driver_buffer[128 + j*DISP_UNIT*Cols + i] << j;
            data[1] += gpio_driver_buffer[15 + j*DISP_UNIT*Rows - i] << (7-j);
            data[0] += gpio_driver_buffer[128 + 15 + j*DISP_UNIT*Rows - i] << (7-j);
            // Prevodi podatke sa ulazne matrice tako da na displeju 16x16 ispis izgleda
            // isto kao matrica na ulazu 
        }
...
/*Na osnovu matrice koja se pro¿ita iscrtava celu matricu na displej veli¿ine 64*/
void printDisp(void)
{
    unsigned char i, j;
    ...
    for(i = 0; i < DISP_UNIT; i++)
    {
        operations[0] = OP_DIGIT0 + i;
        data[0] = 0;

        for(j = 0; j < DISP_UNIT; j++)
            data[0] += gpio_driver_buffer[7 + j*DISP_UNIT*Rows - i] << (7-j);
...
/*Prosle¿uje svim displejima iste podatke, bez pam¿enja stanja*/
void WriteToScreenSame(unsigned char op, unsigned char dat)
{
    unsigned char i;
    WriteGpioPin(LOAD, HIGH);  
...
/*Ispisuje sve 0 na displej*/
void ClearScreen(void)
{
    unsigned char i;
    for(i = 0; i < 8; i++)
        WriteToScreenSame(OP_DIGIT0 + i, 0);
}
...
void Init(void)
{
    ...
    if((Res[0] < '0' ) ||
       ...
       (Res[2] > '9' ))
    {
        printk(KERN_INFO "Bad input params");
        gpio_driver_exit();
    }  

    Rows = Res[0] - '0';
    Cols = Res[2] - '0';
    numOfDisplays = Rows * Cols;
    ...
    if(Rows * Cols > 8)
    {
        ...
        Cols = 1;
        numOfDisplays = 1;
        printk(KERN_INFO "Invalid resolution!");
    }
    
    if((AlwaysInit[0] == 'O' || AlwaysInit[0] == 'o') &&
       (AlwaysInit[1] == 'F' || AlwaysInit[1] == 'f') &&
       (AlwaysInit[2] == 'F' || AlwaysInit[2] == 'f'))
        AlwaysInit = "off";

    // Parsiranje ulaznog parametra za rezoluciju i osve¿avanje promenljivih koje diktiraju
    // na¿in rada

    printk( KERN_INFO "Number of displays is: %hd\n", Rows*Cols);
    printk( KERN_INFO "Current resolutin is: %s\n", Res);
    printk( KERN_INFO "Loop initialization is: %s\n", AlwaysInit);

    WriteToScreenSame(OP_SCANLIMIT, 7);
    WriteToScreenSame(OP_DECODEMODE, 0);
    WriteToScreenSame(OP_DISPLAYTEST,0);
    WriteToScreenSame(OP_INTENSITY, 8);
    WriteToScreenSame(OP_SHUTDOWN, 1);
    ClearScreen();
}
...
int gpio_driver_init(void)
{
    ...
    SetGpioPinDirection(DATA, GPIO_DIRECTION_OUT);
    SetGpioPinDirection(LOAD, GPIO_DIRECTION_OUT);
    SetGpioPinDirection(CLK, GPIO_DIRECTION_OUT);

    Init();

    gpio_driver_buffer = kmalloc(numOfDisplays*DISP_SIZE, GFP_KERNEL);
    ...
    memset(gpio_driver_buffer, 0, numOfDisplays*DISP_SIZE);
    return 0;
...
void gpio_driver_exit(void)
{
    ...
    SetGpioPinDirection(DATA, GPIO_DIRECTION_IN);
    SetGpioPinDirection(LOAD, GPIO_DIRECTION_IN);
    SetGpioPinDirection(CLK, GPIO_DIRECTION_IN);
...
{
    if(AlwaysInit[0] == 'o' && AlwaysInit[1] == 'n')
    {
        WriteToScreenSame(OP_SCANLIMIT, 7);
        WriteToScreenSame(OP_DECODEMODE, 0);
        WriteToScreenSame(OP_DISPLAYTEST,0);
